...
<form action="category_joke_disp1.php?tname=<?php echo $_SESSION[ch];?>">
    ...
    exit();
						...
						for($i=$k;$i>($k-1)&&$i>0;$i--)
						{
							...
							$result = mysqli_query($con,"SELECT question as n FROM $tname WHERE id=$var1"); 
							while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
							{  
								$question=$row['n'] . ' ';
							}
							
							$result = mysqli_query($con,"SELECT answer as n FROM $tname WHERE id=$var1"); 
							while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
							{    
								$answer=$row['n'] . ' ';
							}
		...
		?></small><br></div>
										...
										<?php
										if($_GET['tname']!="knock" and $_GET['tname']!="food" )
										{
										?>
